...
import { BadRequestError, IAuthDocument, IEmailMessageDetails, lowerCase } from '@huseyinkaraman/jobber-shared';

export const read = async (req: Request, res: Response): Promise<void> => {
  let user: IAuthDocument | null = null;
...
export const resendEmail = async (req: Request, res: Response): Promise<void> => {
  ...
  const verificationLink = `${config.CLIENT_URL}/verify-email?v_token=${randomCharacters}`;
